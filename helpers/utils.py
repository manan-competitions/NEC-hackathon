...
def fitness(routes, sim_dg, G, c1, c2, c3, opt_bus, components=False):
    ...
    return (
        c1 * num_ppl
        + c2 / (1 + np.abs(routes.num_buses - opt_bus))
        + c3 / (1 + routes.cum_len)
    )
...
def measureInconveniece(G, num_of_people):
...
def GA(
    ...
    for i in range(iter):
        ...
        best = fit[0][0]
        print(
            f"-- Average: {np.mean([f[1] for f in fit])} Best: {fit[0][1]} Worst: {fit[-1][1]}"
        )
    return best, new_pop
